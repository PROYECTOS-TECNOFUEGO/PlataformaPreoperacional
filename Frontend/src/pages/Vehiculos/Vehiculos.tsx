...
const Vehiculos: FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${api}/vehiculos`)
      .then((res) => {
        const lista: Vehicle[] = (Array.isArray(res.data) ? res.data : []).map((v: any) => ({
          id: v.id ?? undefined,            // si tu JSON usa la placa como id, también sirve
          placa: v.placa,
          tipo: v.tipo,
          modelo: v.modelo,
  ...
  const vehiculosFiltrados = useMemo(
    () => vehiculos.filter((v) => v.placa.toLowerCase().includes(busqueda.toLowerCase())),
    [busqueda, vehiculos]
  );
  ...
  const handleEstadoChange = async (vehiculo: Vehicle, e: SelectChangeEvent<string>) => {
    const next = e.target.value as EstadoVehiculo;

    // ✅ Snapshot profundo para poder revertir
    ...
    try {
      ...
      await axios.patch(`${api}/vehiculos/${encodeURIComponent(String(key))}`, { estado: next });
          ...
          {vehiculosFiltrados.length === 0 ? (
            ...
            vehiculosFiltrados.map((vehiculo, idx) => (
              <Box key={vehiculo.id ?? vehiculo.placa}>
                ...
                {idx !== vehiculosFiltrados.length - 1 && <Divider sx={{ ml: 5 }} />}
              </Box>
            ))
          )}
        </Box>
