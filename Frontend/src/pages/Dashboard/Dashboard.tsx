...
const Dashboard = () => {
  ...
  const opciones = useMemo(() => {
    ...
    const placas = Array.from(
      new Set(formularios.map((f) => f.placa).filter((x) => Boolean(x && x.trim()))),
    );
    const conductores = Array.from(
      new Set(formularios.map((f) => f.conductor).filter((x) => Boolean(x && x.trim()))),
    );
    const estados = Array.from(
      new Set(vehiculos.map((v) => v.estado).filter((x): x is string => Boolean(x && (x as string).trim()))),
    );
  ...
  useEffect(() => {
    setLoading(true);
    Promise.allSettled([axios.get(`${base}/formularios`), axios.get(`${base}/vehiculos`)])
      .then(([fRes, vRes]) => {
  ...
  const {
    ...
    const f = formularios.filter((x) => {
      const texto = `${x.codigo ?? ''} ${x.placa ?? ''} ${x.conductor ?? ''}`.toLowerCase();
      const coincideTexto = !filters.q || texto.includes(filters.q.toLowerCase());
    ...
    const sevenFrom = `${sevenAgo.getFullYear()}-${String(sevenAgo.getMonth() + 1).padStart(
      2,
      '0',
    )}-${String(sevenAgo.getDate()).padStart(2, '0')}`;
    const semanaCount = f.filter((x) => inRange(x.fecha, sevenFrom, filters.to)).length;

    // Mes en curso (según "to")
    const toRef = toDate(filters.to);
    const monthFrom = `${toRef.getFullYear()}-${String(toRef.getMonth() + 1).padStart(2, '0')}-01`;
    const mesCount = f.filter((x) => inRange(x.fecha, monthFrom, filters.to)).length;
    ...
    const spanDays =
      Math.ceil(
        (toDate(filters.to).getTime() - toDate(filters.from).getTime()) / (1000 * 3600 * 24),
      ) + 1;
    ...
    const prevFromStr = `${prevFrom.getFullYear()}-${String(prevFrom.getMonth() + 1).padStart(
      2,
      '0',
    )}-${String(prevFrom.getDate()).padStart(2, '0')}`;
    const prevToStr = `${prevTo.getFullYear()}-${String(prevTo.getMonth() + 1).padStart(
      2,
      '0',
    )}-${String(prevTo.getDate()).padStart(2, '0')}`;
    ...
    const delta =
      prevChecks.pct === 0 && currentChecks.pct === 0
        ...
        : prevChecks.pct === 0
        ? 100
        : Math.round(((currentChecks.pct - prevChecks.pct) / Math.max(prevChecks.pct, 1)) * 100);
    ...
    <PageContainer>
      ...
      <Grid container spacing={2} sx={{ mb: 1 }}>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="today" label="Preoperaciones (hoy)" value={kpiHoy} loading={Boolean(loading)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="date_range" label="Últimos 7 días" value={kpiSemana} loading={Boolean(loading)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="calendar_month" label="Mes en curso" value={kpiMes} loading={Boolean(loading)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard
            icon="check_circle"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="directions_car" label="Vehículos activos" value={activos} loading={Boolean(loading)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="build_circle" label="En mantenimiento" value={mantenimiento} loading={Boolean(loading)} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <KpiCard icon="warning" label="Alertas abiertas" value={alertas} loading={Boolean(loading)} />
        </Grid>
      </Grid>

      <Grid container spacing={2} sx={{ mt: 1 }}>
        <Grid item xs={12} md={8}>
          <TimeSeries title="Tendencia de preoperacionales" data={serieDiaria} />
        </Grid>
