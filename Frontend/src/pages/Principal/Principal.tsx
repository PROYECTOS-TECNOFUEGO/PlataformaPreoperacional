...
type EstadoParcial = 'En curso' | 'Pendiente' | 'Terminado' | 'No aplica' | string;
...
const Principal: FC = () => {
  ...
  const getEstadoDetalle = (r: PrincipalItem) => {
    ...
    if (eg === 'Finalizado') {
      return { chip: { label: 'Finalizado', color: 'success' as const }, detalle: 'Preoperacional finalizado' };
    }

    // En proceso
    if (!r.tieneRelevo) {
      if (ep === 'En curso') return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'Conductor en ruta' };
      if (ep === 'Terminado') return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'Conductor terminó (pendiente cierre)' };
      return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'En curso' };
    }

    // Con relevo
    if (ep === 'En curso' && er === 'Pendiente') {
      return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'Principal en ruta' };
    }
    if (ep === 'Terminado' && er === 'Pendiente') {
      return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'Primer conductor terminado (pendiente relevo)' };
    }
    if (er === 'En curso') {
      return { chip: { label: 'En proceso', color: 'warning' as const }, detalle: 'Relevo en ruta' };
    }

    return { chip: { label: eg, color: 'default' as const }, detalle: `${ep} / ${er}` };
  };

  // Helpers para filtros
  const isFinalizado = (r: PrincipalItem) => (r.estadoGeneral ?? '') === 'Finalizado';
    ...
    axios
      ...
      .then((res) => {
        ...
        const registros: PrincipalItem[] = arr.map((item: any) => ({
          id: item.id || item._id || item.codigo || `${Date.now()}_${Math.random().toString(36).slice(2)}`,
  ...
  const handleFinalizarDesdeTabla = async (row: PrincipalItem) => {
    ...
    const actor = !row.tieneRelevo
      ? 'principal'
      : (row.estadoPrincipal === 'Terminado' ? 'relevo' : 'principal');
        ...
        prev.map((r) => {
          ...
          if (actor === 'principal') {
            return {
              ...
              estadoRelevo: r.tieneRelevo ? (r.estadoRelevo || 'Pendiente') : 'No aplica',
