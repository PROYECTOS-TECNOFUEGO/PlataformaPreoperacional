...
const TimeSeries: FC<Props> = ({ title, data, height = 220 }) => {
  ...
  const tickIdxs =
    tickCount > 1
      ? [...Array(tickCount)].map((_, i) => Math.round((i * (n - 1)) / (tickCount - 1)))
      : [0];
  ...
  return (
    <Card>
      ...
      <CardContent sx={{ pt: 0 }}>
        <Box component="svg" viewBox={`0 0 ${W} ${H}`} sx={{ width: '100%', height }}>
          {/* Ejes */}
          ...
          {[0, 1, 2, 3].map((k) => {
            ...
            return <line key={k} x1={P} y1={yy} x2={W - P} y2={yy} stroke="#eee" />;
          })}
          ...
          {/* Línea */}
          {data.length >= 1 && (
            <path d={lineD} fill="none" stroke={theme.palette.primary.main} strokeWidth={2} />
          )}

          {/* Puntos */}
          {data.map((p, i) => (
            <circle key={i} cx={x(i)} cy={y(p.value)} r={3} fill={theme.palette.primary.main} />
          ))}
          ...
          {tickIdxs.map((idx) => (
            ...
            >
              {data[idx]?.date?.slice(5)}{/* muestra MM-DD */}
            </text>
          ))}

          {/* Max label Y */}
          <text x={P - 8} y={y(maxY)} fontSize={11} textAnchor="end" fill={theme.palette.text.secondary}>
            {maxY}
          </text>
        </Box>
      </CardContent>
